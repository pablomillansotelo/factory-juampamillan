...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${FACTORY_API_URL}/v1/internal-items`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': FACTORY_API_KEY,
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al crear item interno' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en POST /api/factory/v1/internal-items:', error);
    return NextResponse.json(
      { error: error.message || 'Error al crear item interno' },
      { status: 500 }
    );
  }
}
