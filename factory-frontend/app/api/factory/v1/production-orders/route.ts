...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${FACTORY_API_URL}/v1/production-orders`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al obtener órdenes de producción' },
    ...
    console.error('Error en GET /api/factory/v1/production-orders:', error);
    return NextResponse.json(
      { error: error.message || 'Error al obtener órdenes de producción' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al crear orden de producción' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en POST /api/factory/v1/production-orders:', error);
    return NextResponse.json(
      { error: error.message || 'Error al crear orden de producción' },
      { status: 500 }
    );
  }
}
